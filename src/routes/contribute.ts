...
import { COUNTRY_CODES, countryName, FORMATS, SOURCE_KINDS } from "../lib/reference.js";
import { saveImages } from "../lib/uploads.js";
import { ValidationError, type FieldErrors } from "../lib/validation.js";
import { getEditionSummary, listLabels, listReleasesBrief, type DuplicateCandidate } from "../domain/catalog.js";
import {
  acceptProposal, editionAsPayload, getProposal, listProposals, PAYLOAD_FIELD_LABELS, rejectProposal, submitProposal,
...
function editionFields(v: Record<string, any>, errors?: FieldErrors, labels: { name: string }[] = []) {
  return html`
    <fieldset><legend>Edition details</legend>
      <div class="grid-2">
        ...
        ${selectField({ label: "Country", name: "country", value: v.country, options: COUNTRY_CODES.map((c) => ({ value: c, label: countryName(c) })), blank: "Unknown", errors })}
      </div>
      <div class="grid-3">
      ...
      ${textArea({ label: "Distinguishing details", name: "edition_notes", value: v.edition_notes, errors, rows: 3, hint: "What tells this edition apart from others: label colour, sticker, sleeve, runout differences." })}
      ${textArea({ label: "Track listing (this edition)", name: "tracks", value: v.tracks, errors, rows: 6, mono: true, hint: "One per line: position | title | duration. Add a different artist credit with “ // ”, e.g. B1 | Title // Artist | 6:12" })}
...
function duplicatePanel(dups: DuplicateCandidate[]) {
  return html`<section class="panel dup" aria-labelledby="dup-h">
    ...
    <p>These existing editions look similar. If yours is one of them, propose a correction instead.</p>
    <table class="compact"><thead><tr><th>Edition</th><th>Why it matched</th><th>Status</th></tr></thead><tbody>
      ${dups.map((d) => html`<tr><td><a href="/editions/${d.edition.id}" target="_blank" rel="noopener">${d.artist} — ${d.release_title}<br><span class="catno">${d.edition.catalog_number ?? "—"}</span> ${d.edition.label ?? ""} · ${d.edition.format} · ${d.edition.country ?? "?"} · ${d.edition.release_year ?? "?"}</a></td>
        <td><ul class="small">${d.reasons.map((r) => html`<li>${r}</li>`)}</ul></td><td>${verificationBadge(d.edition.verification_status)}</td></tr>`)}
    </tbody></table>
    <label class="check"><input type="checkbox" name="confirm_not_duplicate" value="yes"> <span>I've compared these and my edition is different.</span></label>
...
export function registerContributeRoutes(app: Express, ctx: AppContext) {
  app.get("/contribute", (req, res) => {
    ...
    page(req, res, {
      ...
      body: html`<h1>Contribute to the archive</h1>
        <p>Propose new editions or corrections. Every change needs a source and stays <strong>pending</strong> until a moderator accepts it. Accepted changes are recorded in the edition's revision history with your name and the moderator's.</p>
        <p class="muted small">To propose, open a release and choose “Propose a missing edition”, or open an edition and choose “Propose a correction”.</p>
        <div class="actions"><a class="btn" href="/contribute/new-edition">Propose a new edition</a></div>
        ...
        ${mine.length
          ? html`<div class="table-wrap"><table class="compact"><thead><tr><th>#</th><th>Type</th><th>Release</th><th>Status</th><th>Submitted</th></tr></thead><tbody>
              ${mine.filter((p) => p.proposed_by === u.id).map((p) => html`<tr><td><a href="/proposals/${p.id}">#${p.id}</a></td><td>${p.kind === "new_edition" ? "New edition" : "Correction"}</td><td>${p.release_title}</td><td>${statusBadge(p.status)}</td><td class="small">${p.created_at.slice(0, 10)}</td></tr>`)}
            </tbody></table></div>`
  ...
  const renderNew = (req: Request, res: Response, v: Record<string, any>, errors?: FieldErrors, dups?: DuplicateCandidate[]) => {
    ...
    const releases = listReleasesBrief(ctx.db);
    page(req, res, {
      title: "Propose a new edition",
      nav: "contribute",
      body: html`<h1>Propose a new edition</h1>
        <form method="post" action="/contribute/new-edition" enctype="multipart/form-data" class="form-narrow" novalidate>
          ...
          ${selectField({ label: "Release", name: "release_id", value: v.release_id, options: releases.map((r) => ({ value: String(r.id), label: `${r.artist} — ${r.title}` })), blank: "Choose the release this is an edition of…", errors, required: true, hint: "New releases (not just editions) are a later milestone." })}
          ${editionFields(v, errors, listLabels(ctx.db))}
  ...
  app.get("/contribute/new-edition", (req, res) => renderNew(req, res, { release_id: req.query.release_id }));
  ...
  app.post("/contribute/new-edition", (req, res) => {
    ...
    if (!releaseId) return renderNew(req, res, req.body, { release_id: "Choose a release." });
    try {
      const imagePaths = saveImages(ctx.config.uploadDir, "proposals", req.files as Express.Multer.File[]).map((path) => ({ path, caption: null }));
      const r = submitProposal(ctx.db, ctx.clock, user, { kind: "new_edition", release_id: releaseId, target_edition_id: null, body: req.body, imagePaths });
  ...
  const renderCorrection = (req: Request, res: Response, editionId: number, v: Record<string, any>, errors?: FieldErrors) => {
    requireRole(req.state.user, "contributor");
    ...
    page(req, res, {
      ...
      nav: "contribute",
      body: html`<nav class="crumbs"><a href="/editions/${e.id}">${title} <span class="catno">${e.catalog_number ?? ""}</span></a> / <span>Correction</span></nav>
        ...
        <form method="post" action="/contribute/correction?edition_id=${e.id}" enctype="multipart/form-data" class="form-narrow" novalidate>
          ${csrf(req)}${errorSummary(errors)}
          ${editionFields(v, errors)}
          ${sourceFields(v, errors)}
          <div class="actions"><button class="btn btn-primary" type="submit">Submit correction for review</button><a class="btn btn-quiet" href="/editions/${e.id}">Cancel</a></div>
        </form>`,
    }, errors ? 422 : 200);
  };

  app.get("/contribute/correction", (req, res) => {
    const id = Number(req.query.edition_id);
    renderCorrection(req, res, id, editionAsPayload(ctx.db, id));
  });
  ...
  app.post("/contribute/correction", (req, res) => {
    ...
    try {
      const imagePaths = saveImages(ctx.config.uploadDir, "proposals", req.files as Express.Multer.File[]).map((path) => ({ path, caption: null }));
      const r = submitProposal(ctx.db, ctx.clock, user, { kind: "correction", release_id: e.release_id, target_edition_id: editionId, body: req.body, imagePaths });
      if (r.ok) {
        addFlash(req, "success", `Correction #${r.proposalId} submitted. The edition is unchanged until a moderator accepts it.`);
        res.redirect(303, `/proposals/${r.proposalId}`);
      }
    } catch (err) {
      if (err instanceof ValidationError) return renderCorrection(req, res, editionId, req.body, err.fields);
      throw err;
    }
  });
  ...
  app.get("/proposals/:id", (req, res) => {
    ...
    page(req, res, {
      ...
      body: html`<nav class="crumbs"><a href="${u.roles.includes("moderator") ? "/moderate" : "/contribute"}">${u.roles.includes("moderator") ? "Moderation queue" : "Contribute"}</a> / <span>Proposal #${p.id}</span></nav>
        <h1>${p.kind === "new_edition" ? "New edition" : "Correction"} for ${p.release_title} ${statusBadge(p.status)}</h1>
        ...
        ${p.kind === "correction" ? html`<p><a href="/editions/${p.target_edition_id}">View the current edition</a></p>` : ""}
        ${p.resulting_edition_id ? html`<p><a class="btn btn-quiet" href="/editions/${p.resulting_edition_id}">View the edition</a></p>` : ""}
        ${p.duplicate_ids.length ? html`<p class="panel dup">The contributor was shown possible duplicates and confirmed this is different: ${p.duplicate_ids.map((id: number) => html`<a href="/editions/${id}">edition #${id}</a> `)}</p>` : ""}
        ...
        ${p.changes.length
          ...
          : html`<p class="muted">${p.status === "pending" ? "No differences from the current entry (it may already have been corrected)." : "Changes are recorded in the edition's revision history."}</p>`}
        <h2>Evidence</h2>
        ...
        ${canReview
          ? html`<div class="panel"><h2>Review</h2>
              <p class="small">Accepting marks this edition “Moderator-reviewed” against the cited source. It does not claim the entry is certain.</p>
      ...
      body: html`<h1>Moderation queue</h1>
        ...
        ${list.length
          ? html`<div class="table-wrap"><table class="compact"><thead><tr><th>#</th><th>Type</th><th>Release</th><th>Proposed by</th><th>Status</th><th>Submitted</th></tr></thead><tbody>
              ${list.map((p) => html`<tr><td><a href="/proposals/${p.id}">#${p.id}</a></td><td>${p.kind === "new_edition" ? "New edition" : "Correction"}</td><td>${p.release_title}</td><td>${p.proposed_by_name}${p.proposed_by === u.id ? raw(' <span class="muted small">(you)</span>') : ""}</td><td>${statusBadge(p.status)}</td><td class="small">${p.created_at.slice(0, 10)}</td></tr>`)}
            </tbody></table></div>`
  ...
  app.post("/proposals/:id/accept", (req, res) => {
    const editionId = acceptProposal(ctx.db, ctx.clock, req.state.user, idParam(req), String(req.body.review_note ?? "").trim() || null);
    addFlash(req, "success", "Proposal accepted and applied. The revision is recorded.");
    res.redirect(303, `/editions/${editionId}`);
  });
